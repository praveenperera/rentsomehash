...
description: "Use this if you already run StartOS and want to reuse that box for DATUM before connecting rented hashpower."
slug: startos
order: 2
stage: node
...
> Use a stable hostname for rented hashpower. MiningRigRentals does not refund rentals pointed at IP-based pools, and a changing home IP can strand paid rental time on a dead endpoint. Use DDNS, HashGG, Start Tunnel with a hostname, or PickHash's DuckDNS option before spending sats.

There are two broad ways to expose the service:

- Forward a port from your router and give the endpoint a DDNS hostname
...
## 1. Install [BIP-110](https://bip110.org/) on StartOS
...
Once it is installed, make sure it is the service you are actually exposing to rented hashpower, not some other node endpoint by accident.
...
## 4. Record the tunnel endpoint

When Start Tunnel is in front of the gateway, your hashpower provider should use:

```text
stratum+tcp://your-vps-ip:23334
```

Replace `your-vps-ip` with the public IP of the VPS running Start Tunnel. PickHash can give this raw IP a stable DuckDNS name during its setup wizard.
...
The rental provider still needs a stable public endpoint, not the private LAN address.
...
Do not point paid rentals at a rotating home IP.
...
If your endpoint is a raw IP, enable PickHash's DuckDNS option so MiningRigRentals treats it as a hostname.
...
- Start Tunnel gives a hashpower provider a stable public endpoint without handing over your literal home IP
